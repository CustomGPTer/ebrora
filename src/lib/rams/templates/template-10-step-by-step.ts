    ...
    sections: [
      ...
      // SECTION 3 — Portrait Supporting Information (not a full MS — method is in the table)
      {
        ...
        children: [
          ...
          h.subHeading('1. Interfaces & Coordination'), h.bodyText(content.interfaces),
          h.subHeading('2. Temporary Works'), h.bodyText(content.temporaryWorks),
          h.subHeading('3. PPE Summary'), h.bodyText(content.ppeRequirements),
          h.subHeading('4. Environmental Considerations'), h.bodyText(content.environmentalConsiderations),
          h.subHeading('5. Waste Management'), h.bodyText(content.wasteManagement),
          h.subHeading('6. Emergency Procedures'), h.bodyText(content.emergencyProcedures),
          h.subHeading('7. Welfare Facilities'), h.bodyText(content.welfareFacilities),
          h.subHeading('8. Competency Requirements'), h.bodyText(content.competencyRequirements),
...
function buildIntegratedTable(c: Template10Content): Table {
  ...
  return new Table({ width: { size: tw, type: WidthType.DXA }, columnWidths: cols, rows: [
    new TableRow({ children: [
      ...
      h.headerCell('Hazards at Step', cols[2], { fontSize: 10 }),
      ...
      h.headerCell('Res.', cols[6], { fontSize: 10, alignment: AlignmentType.CENTER }),
