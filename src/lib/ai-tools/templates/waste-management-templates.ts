...
function buildCover(slug: WasteTemplateSlug, p: Palette, d: any): (Paragraph | Table)[] {
  if (slug === 'full-compliance') return [gap(200), new Table({ width: { size: W, type: WidthType.DXA }, columnWidths: [W], rows: [new TableRow({ children: [new TableCell({ borders: noBorders, width: { size: W, type: WidthType.DXA }, shading: { fill: p.primary, type: ShadingType.CLEAR }, margins: { top: 500, bottom: 500, left: 300, right: 300 }, children: [new Paragraph({ spacing: { after: 100 }, children: [new TextRun({ text: 'SITE WASTE', bold: true, font: p.font, size: 48, color: 'FFFFFF' })] }), new Paragraph({ spacing: { after: 60 }, children: [new TextRun({ text: 'MANAGEMENT PLAN', bold: true, font: p.font, size: 36, color: '80CBC4' })] }), new Paragraph({ children: [new TextRun({ text: `${d.projectName || ''}  |  ${d.documentRef || ''}`, font: p.font, size: 20, color: 'B2DFDB' })] })] })] })] }), gap(120), new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: 'EPA 1990 s.34 — Duty of Care', bold: true, font: p.font, size: 18, color: p.accent })] }), gap(300)];
  if (slug === 'corporate') return [gap(400), new Table({ width: { size: W, type: WidthType.DXA }, columnWidths: [W], rows: [new TableRow({ children: [new TableCell({ borders: noBorders, width: { size: W, type: WidthType.DXA }, shading: { fill: p.primary, type: ShadingType.CLEAR }, margins: { top: 400, bottom: 400, left: 300, right: 300 }, children: [new Paragraph({ spacing: { after: 80 }, children: [new TextRun({ text: 'WASTE MANAGEMENT PLAN', bold: true, font: p.font, size: 44, color: 'FFFFFF' })] }), new Paragraph({ children: [new TextRun({ text: d.projectName || '', font: p.font, size: 22, color: 'BFDBFE' })] })] })] })] }), gap(300)];
  return [];
}
