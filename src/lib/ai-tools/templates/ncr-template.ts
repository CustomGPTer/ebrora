        ...
        children: [
          ...
          ...(Array.isArray(content.correctiveActions) ? [new Table({ width: { size: W, type: WidthType.DXA }, rows: [
            new TableRow({ children: [
              ...
              h.headerCell('Responsible', Math.round(W * 0.15), { fontSize: 13 }),
              h.headerCell('Target', Math.round(W * 0.11), { fontSize: 13 }),
              h.headerCell('Priority', Math.round(W * 0.09), { fontSize: 13 }),
              h.headerCell('Status', Math.round(W * 0.09), { fontSize: 13 }),
              h.headerCell('Verification', W - Math.round(W * 0.80), { fontSize: 13 }),
            ] }),
            ...content.correctiveActions.map((a: any, i: number) => new TableRow({ children: [
              ...
              h.dataCell(a.action || '', Math.round(W * 0.30), { fontSize: 13 }),
              h.dataCell(a.responsiblePerson || '', Math.round(W * 0.15), { fontSize: 13 }),
              h.dataCell(a.targetDate || '', Math.round(W * 0.11), { fontSize: 13 }),
              h.dataCell(a.priority || '', Math.round(W * 0.09), { fontSize: 13, bold: true }),
              h.dataCell(a.status || 'Open', Math.round(W * 0.09), { fontSize: 13, bold: true }),
              h.dataCell(a.verificationMethod || '', W - Math.round(W * 0.80), { fontSize: 13 }),
            ] })),
          ] })] : []),
        ],
      },
        ...
        children: [
          ...
          ...(Array.isArray(content.preventiveActions) ? [new Table({ width: { size: W, type: WidthType.DXA }, rows: [
            new TableRow({ children: [
              ...
              h.headerCell('Responsible', Math.round(W * 0.18), { fontSize: 14 }),
              h.headerCell('Target Date', Math.round(W * 0.14), { fontSize: 14 }),
              h.headerCell('Priority', Math.round(W * 0.10), { fontSize: 14 }),
              h.headerCell('Status', W - Math.round(W * 0.86), { fontSize: 14 }),
            ] }),
            ...content.preventiveActions.map((a: any, i: number) => new TableRow({ children: [
              ...
              h.dataCell(a.action || '', Math.round(W * 0.38), { fontSize: 14 }),
              h.dataCell(a.responsiblePerson || '', Math.round(W * 0.18), { fontSize: 14 }),
              h.dataCell(a.targetDate || '', Math.round(W * 0.14), { fontSize: 14 }),
              h.dataCell(a.priority || '', Math.round(W * 0.10), { fontSize: 14, bold: true }),
              h.dataCell(a.status || 'Open', W - Math.round(W * 0.86), { fontSize: 14, bold: true }),
            ] })),
          ] })] : []),
          h.spacer(200),
