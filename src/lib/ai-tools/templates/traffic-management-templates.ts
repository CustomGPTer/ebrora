...
function buildCover(slug: TrafficTemplateSlug, p: Palette, d: any): (Paragraph | Table)[] {
  if (slug === 'full-chapter8') return [new Table({ width: { size: W, type: WidthType.DXA }, columnWidths: [W], rows: [new TableRow({ children: [new TableCell({ borders: noBorders, width: { size: W, type: WidthType.DXA }, shading: { fill: p.primary, type: ShadingType.CLEAR }, margins: { top: 300, bottom: 300, left: 300, right: 300 }, children: [new Paragraph({ spacing: { after: 100 }, children: [new TextRun({ text: 'TRAFFIC MANAGEMENT', bold: true, font: p.font, size: 48, color: 'FFFFFF' })] }), new Paragraph({ spacing: { after: 60 }, children: [new TextRun({ text: 'PLAN', bold: true, font: p.font, size: 36, color: 'A7F3D0' })] }), new Paragraph({ spacing: { after: 40 }, children: [new TextRun({ text: d.projectName || '', font: p.font, size: 22, color: 'D1FAE5' })] }), new Paragraph({ children: [new TextRun({ text: `${d.documentRef || ''}  |  ${d.planDate || ''}`, font: p.font, size: 20, color: 'D1FAE5' })] })] })] })] }), gap(120), new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: 'Chapter 8 / HSG144 Compliant', bold: true, font: p.font, size: 18, color: p.accent })] }), gap(150)];
  if (slug === 'formal-highways') return [gap(600), new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 }, children: [new TextRun({ text: 'TRAFFIC MANAGEMENT PLAN', bold: true, font: p.font, size: 48, color: p.primary })] }), new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 60 }, border: { bottom: { style: BorderStyle.SINGLE, size: 4, color: p.accent, space: 8 } }, children: [new TextRun({ text: d.projectName || '', font: p.font, size: 28, color: p.dark })] }), new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 40 }, children: [new TextRun({ text: `${d.documentRef || ''}  |  ${d.planDate || ''}`, font: p.font, size: 20, color: p.mid })] }), gap(150)];
  if (slug === 'site-plan') return [new Table({ width: { size: W, type: WidthType.DXA }, columnWidths: [W], rows: [new TableRow({ children: [new TableCell({ borders: noBorders, width: { size: W, type: WidthType.DXA }, shading: { fill: p.primary, type: ShadingType.CLEAR }, margins: { top: 300, bottom: 300, left: 300, right: 300 }, children: [new Paragraph({ spacing: { after: 80 }, children: [new TextRun({ text: 'TM PLAN', bold: true, font: p.font, size: 44, color: 'FFFFFF' })] }), new Paragraph({ children: [new TextRun({ text: d.projectName || '', font: p.font, size: 22, color: 'BFDBFE' })] })] })] })] }), gap(150)];
  return [];
}
