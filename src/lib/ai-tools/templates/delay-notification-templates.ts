...
export async function buildDelayTemplateDocument(content: any, templateSlug: DelayTemplateSlug): Promise<Document> {
  ...
  children.push(buildInfoTable(p, [['Document Reference', d.documentRef || ''], ['Date', d.notificationDate || ''], ['Contract Form', d.contractForm || ''], ['Contract Reference', d.contractRef || ''], ['Project', d.projectName || ''], ['Site Address', d.siteAddress || ''], ['Addressed To', d.addressee || '']]));
  children.push(gap());
  if (d.delayEvent) {
    children.push(sectionHead(templateSlug, p, 2, 'Delay Event'));
    for (const para of (d.delayEvent as string).split(/\n\n?/).filter(Boolean)) children.push(bodyPara(p, para));
    children.push(gap());
  }
  ...
  if (d.mitigationMeasures) {
    children.push(sectionHead(templateSlug, p, 5, 'Mitigation Measures'));
    for (const para of (d.mitigationMeasures as string).split(/\n\n?/).filter(Boolean)) children.push(bodyPara(p, para));
    children.push(gap());
  }
  if (d.costEntitlement) {
    children.push(sectionHead(templateSlug, p, 6, 'Cost Entitlement'));
    for (const para of (d.costEntitlement as string).split(/\n\n?/).filter(Boolean)) children.push(bodyPara(p, para));
    children.push(gap());
  }
  if (d.priorNotices) {
    children.push(sectionHead(templateSlug, p, 7, 'Prior Notices & Early Warnings'));
    for (const para of (d.priorNotices as string).split(/\n\n?/).filter(Boolean)) children.push(bodyPara(p, para));
    children.push(gap());
  }
  ...
  const hdr = new Header({ children: [new Paragraph({ border: { bottom: { style: BorderStyle.SINGLE, size: 4, color: p.accent, space: 4 } }, children: [new TextRun({ text: 'DELAY NOTIFICATION', bold: true, font: p.font, size: 17, color: p.primary }), new TextRun({ text: `\t${d.documentRef || ''}`, font: p.font, size: 16, color: p.mid })], tabStops: [{ type: TabStopType.RIGHT, position: TabStopPosition.MAX }] })] });
