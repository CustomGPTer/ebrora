...
function coverPage(d: CfData, accentColor: string, label: string) {
  return [
    ...
    h.spacer(200),
    new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: 'Generated by Ebrora — www.ebrora.com', size: SM, color: '999999', italic: true })] }),
  ];
}
...
function buildT1(d: CfData): Document {
  ...
  sections.push(h.approvalTable(W, (d.approvalChain || []).map((a: any) => ({ role: a.role, name: a.name, date: a.date }))));
