...
export function buildCoverPageChildren(opts: CoverPageOpts): (Paragraph | Table)[] {
  ...
  // Spacing after title (Ebrora branding only in header/footer)
  elements.push(new Paragraph({ spacing: { before: 0, after: 480 }, children: [] }));
...
// ─── Inline title block (no separate cover page) ────────────────────────────

export interface InlineTitleOpts {
  documentLabel: string;
  accentHex: string;
  classification?: string;
}

/** Builds accent bar + big title block that sits at the top of page 1 (no page break) */
export function buildInlineTitleBlock(opts: InlineTitleOpts): (Paragraph | Table)[] {
  const elements: (Paragraph | Table)[] = [];

  // Classification band (optional)
  if (opts.classification) {
    elements.push(new Paragraph({
      alignment: AlignmentType.CENTER,
      spacing: { before: 0, after: 0 },
...
/**
 * Build a premium Document WITHOUT a separate cover page.
 * Title block is merged into the top of the first body section.
 */
export function buildPremiumDocumentInline(
  opts: InlineTitleOpts,
  bodySections: Array<Array<Paragraph | Table>>
): Document {
  const titleBlock = buildInlineTitleBlock(opts);

  // Merge title block into first body section
  const firstSection = [...titleBlock, ...(bodySections[0] || [])];
  const remainingSections = bodySections.slice(1);
  ...
  return new Document({
    ...
    sections: [
      {
        properties: { ...h.PORTRAIT_SECTION },
        headers: { default: h.ebroraHeader(opts.documentLabel) },
        footers: { default: h.ebroraFooter() },
        children: firstSection,
      },
      ...remainingSections.map(children => ({
        properties: { ...h.PORTRAIT_SECTION },
        headers: { default: h.ebroraHeader(opts.documentLabel) },
        footers: { default: h.ebroraFooter() },
        children,
