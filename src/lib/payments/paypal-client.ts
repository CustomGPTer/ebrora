    ...
    `${PAYPAL_BASE}/v1/billing/subscriptions`,
    {
      ...
      body: JSON.stringify({
        ...
        application_context: {
          ...
          return_url: returnUrl || `${baseUrl}/pricing?subscribed=true`,
...
export async function reviseSubscription(
  ...
  const response = await fetch(
    `${PAYPAL_BASE}/v1/billing/subscriptions/${subscriptionId}/revise`,
    {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${accessToken}`,
      ...
      body: JSON.stringify({
        ...
        application_context: {
          ...
          locale: 'en-GB',
          user_action: 'SUBSCRIBE_NOW',
          return_url: returnUrl || `${baseUrl}/pricing?upgraded=true`,
          cancel_url: `${baseUrl}/pricing`,
        },
      }),
    }
  ...
  if (!response.ok) {
    const error = await response.text();
    throw new Error(`Failed to revise subscription: ${error}`);
  }
  ...
  if (!approveLink) {
    throw new Error('No approval link in PayPal revise response');
  }
