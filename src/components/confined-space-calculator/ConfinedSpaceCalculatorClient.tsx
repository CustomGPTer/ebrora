...
async function exportPDF(
  ...
  if (!isEnclosed || !hasRisks) {
    ...
    for (const section of SCORING_SECTIONS) {
      ...
      // Check page space
      if (y + 8 + section.options.length * 4 > 275) {
        doc.addPage();
      ...
      for (let i = 0; i < section.options.length; i++) {
        ...
        const isSelected = scores.includes(opt.weight) && (
          section.mode === "multi"
            ? sectionScores[section.id]?.filter(s => s === opt.weight).length > 0
            : scores[0] === opt.weight
        );
        // For multi sections, check if this specific index was selected
        const selected = section.mode === "multi"
          ? (sectionScores[`${section.id}_indices`] as unknown as number[] || []).includes(i)
          : (sectionScores[`${section.id}_selected`] as unknown as number) === i;

        if (selected) {
          doc.setFillColor(235, 250, 240); doc.rect(M, y - 2, CW, 4, "F");
        ...
        doc.text(`${opt.label}`, M + 10, y);
        doc.text(`${opt.weight}`, M + CW - 8, y);
        y += 4;
      }
      y += 3;
    }
    ...
    if (cat) {
      ...
      doc.text(`${cat.category} Requirements Checklist`, M, y); y += 5;

      const reqs = CATEGORY_REQUIREMENTS[cat.category];
      const colLbl = 55; const colReq = CW - colLbl - 18; const rH = 0;
      ...
      for (const rl of REQUIREMENT_LABELS) {
        ...
        // Checklist tick
        const checked = checklist[rl.key];
        doc.text(checked ? "[X]" : "[ ]", M + colLbl + colReq + 5, y + 3.5);
        y += rowH;
      }
    }
  }
  ...
  for (let p = 1; p <= pc; p++) {
    ...
    doc.text("Assessment per Confined Spaces Regulations 1997 and C&G 6160 framework. This document does not replace a site-specific risk assessment.", M, 290);
...
export default function ConfinedSpaceCalculatorClient() {
  ...
  const [hasRisks, setHasRisks] = useState<boolean | null>(null);

  // Section selections: for "single" = index of selected option (-1 = none), for "multi" = Set of selected indices
  ...
  const isConfinedSpace = isEnclosed === true && hasRisks === true;
  const notConfinedSpace = isEnclosed === false || hasRisks === false;
  ...
  const handleMultiToggle = useCallback((sectionId: string, optionIndex: number) => {
    setMultiSelections(prev => {
      ...
      // If no risks selected, update gate
      if (sectionId === "foreseeable_risks") {
        setHasRisks(current.size > 0 ? true : null);
      }
  ...
  const clearAll = useCallback(() => {
    setIsEnclosed(null); setHasRisks(null);
    setSingleSelections({}); setMultiSelections({});
    setChecklist({});
  ...
  const handleExport = useCallback(async () => {
    setExporting(true);
    try {
      // Build PDF section data
      const pdfScores: Record<string, number[]> = {};
      for (const section of SCORING_SECTIONS) {
        if (section.mode === "single") {
          const idx = singleSelections[section.id] ?? -1;
          pdfScores[section.id] = idx >= 0 ? [section.options[idx].weight] : [];
          pdfScores[`${section.id}_selected`] = [idx] as unknown as number[];
        } else {
          const sel = multiSelections[section.id] || new Set<number>();
          pdfScores[section.id] = [...sel].map(i => section.options[i].weight);
          pdfScores[`${section.id}_indices`] = [...sel] as number[];
        }
      }
      await exportPDF(
        { company, site, manager, assessedBy, date: assessDate, csName, completedBy, completedDate, csQualifications },
        pdfScores, isEnclosed === true, hasRisks === true, totalScore, cat, checklist,
      );
    } finally { setExporting(false); }
  }, [company, site, manager, assessedBy, assessDate, csName, completedBy, completedDate, csQualifications, singleSelections, multiSelections, isEnclosed, hasRisks, totalScore, cat, checklist]);

  const hasData = isEnclosed !== null;
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          {
            label: "Status",
            value: notConfinedSpace ? "Not a Confined Space" : isConfinedSpace ? "Confirmed Confined Space" : "Awaiting Assessment",
            sub: notConfinedSpace ? "Regulation 1(2) not met" : isConfinedSpace ? "Proceed with scoring" : "Answer gate questions",
            bg: notConfinedSpace ? "bg-gray-50" : isConfinedSpace ? "bg-blue-50" : "bg-slate-50",
            border: notConfinedSpace ? "border-gray-200" : isConfinedSpace ? "border-blue-200" : "border-slate-200",
            text: notConfinedSpace ? "text-gray-700" : isConfinedSpace ? "text-blue-800" : "text-slate-700",
            dot: notConfinedSpace ? "bg-gray-400" : isConfinedSpace ? "bg-blue-500" : "bg-slate-400",
          },
          {
            label: "Total Score",
            value: isConfinedSpace ? `${totalScore}` : "--",
            sub: isConfinedSpace ? `of ${maxPossible} max` : "N/A",
            bg: "bg-slate-50", border: "border-slate-200", text: "text-slate-800", dot: "bg-slate-500",
          },
          {
            label: "Category",
            value: cat ? cat.category : "--",
            sub: cat ? cat.label : "Pending",
            bg: cat ? cat.colour : "bg-gray-50",
            border: cat ? cat.borderColour : "border-gray-200",
            text: cat ? cat.textColour : "text-gray-500",
            dot: cat ? (cat.category === "NC1" ? "bg-green-500" : cat.category === "NC2" ? "bg-amber-500" : "bg-red-500") : "bg-gray-400",
          },
          {
            label: "Checklist",
            value: cat ? `${Object.values(checklist).filter(Boolean).length} / ${REQUIREMENT_LABELS.length}` : "--",
            sub: cat ? (Object.values(checklist).filter(Boolean).length === REQUIREMENT_LABELS.length ? "Complete" : "Incomplete") : "N/A",
        ...
        <div className="bg-gray-50 border-2 border-gray-300 rounded-xl p-5">
          ...
          <div className="text-sm text-gray-600">
            {isEnclosed === false
              ...
              : "No foreseeable specified risks are present. Under Regulation 1(2), a confined space must have a reasonably foreseeable specified risk. Standard risk assessment procedures apply but confined space entry controls are not required."}
          </div>
        </div>
      )}
      ...
      {showSettings && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[
            { l: "Confined Space Name/Ref", v: csName, s: setCsName },
            ...
            { l: "CS Qualification(s)", v: csQualifications, s: setCsQualifications },
          ].map(f => (
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-5">
        <h3 className="text-sm font-bold text-gray-800">Gate Questions — Confined Space Definition</h3>
        <p className="text-xs text-gray-500 -mt-2">Both conditions must be met for this to be classified as a confined space under the Confined Spaces Regulations 1997.</p>
          ...
          <div className="flex gap-2">
            {([true, false] as const).map(v => (
              <button key={String(v)} onClick={() => setIsEnclosed(v)}
                className={`px-4 py-2 text-sm font-medium rounded-lg border transition-colors ${isEnclosed === v
                  ? (v ? "bg-blue-100 border-blue-300 text-blue-800" : "bg-gray-100 border-gray-300 text-gray-800")
        ...
        {/* Q2: Risks — only show if enclosed */}
        {isEnclosed === true && (
          <div>
            <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-wide mb-2">Are any of the foreseeable specified risks below present?</label>
            <p className="text-xs text-gray-400 mb-2">If none are present, this is not a confined space. Select the specific risks in the scoring section below.</p>
            <div className="flex gap-2">
              {([true, false] as const).map(v => (
                <button key={String(v)} onClick={() => {
                  setHasRisks(v);
                  if (!v) setMultiSelections(prev => ({ ...prev, foreseeable_risks: new Set() }));
                }}
                  className={`px-4 py-2 text-sm font-medium rounded-lg border transition-colors ${hasRisks === v
                    ? (v ? "bg-blue-100 border-blue-300 text-blue-800" : "bg-gray-100 border-gray-300 text-gray-800")
                    : "bg-white border-gray-200 text-gray-500 hover:bg-gray-50"}`}>
                  {v ? "Yes" : "No"}
                </button>
      ...
      {/* Scoring Sections — only show if confirmed confined space */}
      {isConfinedSpace && (
              ...
              <div key={section.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div className="p-3 space-y-1.5">
                  {section.options.map((opt, idx) => {
                    const isSelected = section.mode === "single"
                      ? singleSelections[section.id] === idx
                      : (multiSelections[section.id] || new Set()).has(idx);
                    return (
                      <button
                        key={idx}
                        onClick={() => section.mode === "single" ? handleSingleSelect(section.id, idx) : handleMultiToggle(section.id, idx)}
                        className={`w-full text-left px-3 py-2.5 rounded-lg border transition-all flex items-center justify-between gap-3 ${isSelected
                          ? "bg-ebrora-light/60 border-ebrora/40 shadow-sm"
                          : "bg-white border-gray-100 hover:bg-gray-50"}`}
                      >
                        <div className="flex items-center gap-2.5">
                          <div className={`w-4 h-4 rounded ${section.mode === "multi" ? "" : "rounded-full"} border-2 flex items-center justify-center transition-colors ${isSelected ? "border-ebrora bg-ebrora" : "border-gray-300 bg-white"}`}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              ...
              <div className="space-y-2">
                {SCORING_SECTIONS.map(section => {
                  ...
                  const maxSection = section.mode === "multi"
                    ? section.options.reduce((a, o) => a + o.weight, 0)
                    : Math.max(...section.options.map(o => o.weight));
          ...
          {cat && (
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className={`px-4 py-3 ${cat.colour} ${cat.borderColour} border-b`}>
                ...
                <p className={`text-[11px] mt-0.5 opacity-70 ${cat.textColour}`}>Tick off each requirement as it is confirmed in place.</p>
              </div>
              <div className="divide-y divide-gray-50">
                {REQUIREMENT_LABELS.map(rl => {
                  const req = CATEGORY_REQUIREMENTS[cat.category][rl.key];
                  const checked = checklist[rl.key] || false;
                  return (
                    <button key={rl.key} onClick={() => toggleChecklist(rl.key)} className={`w-full text-left px-4 py-3 flex items-start gap-3 transition-colors ${checked ? "bg-green-50/40" : "hover:bg-gray-50"}`}>
                      <div className={`mt-0.5 w-4 h-4 rounded border-2 flex-shrink-0 flex items-center justify-center transition-colors ${checked ? "border-green-600 bg-green-600" : "border-gray-300 bg-white"}`}>
                      ...
                      <div>
                        ...
                        <p className={`text-sm mt-0.5 ${checked ? "text-gray-500 line-through" : "text-gray-700"}`}>{req}</p>
                      </div>
                    </button>
                  );
                })}
      ...
      <div className="pt-4 border-t border-gray-100"><p className="text-[11px] text-gray-400 leading-relaxed max-w-xl">
        Confined space categorisation per the Confined Spaces Regulations 1997 and City & Guilds 6160 framework. This calculator supports risk assessment but does not replace a competent person&apos;s judgement. All confined space entry requires a valid permit to work. White-label PDF.</p></div>
    </div>
  );
}
