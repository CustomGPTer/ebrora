...
import { CONFINED_SPACES_TEMPLATE_ORDER, CONFINED_SPACES_FREE_TEMPLATES, ConfinedSpacesTemplateSlug } from '@/lib/confined-spaces/types';
import ConfinedSpacesTemplatePreviewModal from './ConfinedSpacesTemplatePreviewModal';

interface Props { onSelect: (slug: ConfinedSpacesTemplateSlug) => void; }

export default function ConfinedSpacesTemplatePicker({ onSelect }: Props) {
  ...
  const canAccess = (slug: ConfinedSpacesTemplateSlug): boolean => {
    ...
    return CONFINED_SPACES_FREE_TEMPLATES.includes(slug);
  };
  const getStatus = (slug: ConfinedSpacesTemplateSlug): 'available' | 'locked-auth' | 'locked-upgrade' => {
    if (!isAuthenticated) return 'locked-auth';
    if (canAccess(slug)) return 'available';
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-grid-5">
        {CONFINED_SPACES_TEMPLATE_ORDER.map((slug) => {
          const tpl = CONFINED_SPACES_TEMPLATE_CONFIGS[slug];
          const st = getStatus(slug); const locked = st !== 'available';
          const isFree = CONFINED_SPACES_FREE_TEMPLATES.includes(slug);
          return (
            <button key={slug} type="button" className={`tpl-card ${locked ? 'tpl-card--locked' : ''}`} onClick={() => handleClick(slug)}>
              <div className="tpl-card-thumb">
                ...
                <span className="tpl-card-preview-pill" role="button" tabIndex={0} onClick={(e) => { e.stopPropagation(); setPreviewSlug(slug); }} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); setPreviewSlug(slug); } }} title={`Preview all ${tpl.pageCount} pages`}>
              ...
              <div className="tpl-card-footer">
                {st === 'available' && <span className="tpl-card-cta">Use Template &rarr;</span>}
                {st === 'locked-auth' && <span className="tpl-card-cta tpl-card-cta--locked">Sign In to Access</span>}
                {st === 'locked-upgrade' && <span className="tpl-card-cta tpl-card-cta--upgrade">Upgrade Plan</span>}
              </div>
            </button>
          );
        })}
      </div>
      {previewSlug && <ConfinedSpacesTemplatePreviewModal template={CONFINED_SPACES_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />}
    </div>
  );
}
