...
import { CE_TEMPLATE_CONFIGS } from '@/lib/ce/template-config';
import { CE_TEMPLATE_ORDER } from '@/lib/ce/template-config';
import type { CeTemplateSlug } from '@/lib/ce/types';
import CeTemplatePreviewModal from './CeTemplatePreviewModal';

interface CeTemplatePickerProps {
  onSelect: (slug: CeTemplateSlug) => void;
}

export default function CeTemplatePicker({ onSelect }: CeTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<CeTemplateSlug | null>(null);
  ...
  const handleTemplateClick = (slug: CeTemplateSlug) => {
    const templateStatus = getTemplateStatus();
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/ce-notification-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: CeTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a Compensation Event Notification Template</h2>
        {!isAuthenticated && (
          <p>Sign in to access templates and start generating scope documents</p>
        )}
        {isAuthenticated && !isPaid && (
          <p>Compensation Event Notification Builder is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>
        )}
        {isAuthenticated && isPaid && (
      ...
      <div className="template-grid-3">
        {CE_TEMPLATE_ORDER.map((slug) => {
          const template = CE_TEMPLATE_CONFIGS[slug];
      ...
      {/* Preview modal */}
      {previewSlug && (
        <CeTemplatePreviewModal
          template={CE_TEMPLATE_CONFIGS[previewSlug]}
