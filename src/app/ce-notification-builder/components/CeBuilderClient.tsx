...
import CeTemplatePicker from './CeTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function CeBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<CeTemplateSlug | null>(null);
  const toolConfig = AI_TOOL_CONFIGS['ce-notification'];

  const handleTemplateSelect = (slug: CeTemplateSlug) => {
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <CeTemplatePicker onSelect={handleTemplateSelect} />
      </div>
    );
  }

  // Step 2+: Template selected → show the AI builder with back button
  const templateConfig = CE_TEMPLATE_CONFIGS[selectedTemplate];
  ...
  return (
    <div>
      ...
      <AiToolBuilderClient
        ...
        ceTemplateSlug={selectedTemplate}
      />
    </div>
  );
}
