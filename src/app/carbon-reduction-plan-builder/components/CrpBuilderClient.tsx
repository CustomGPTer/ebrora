...
import CrpTemplatePicker from './CrpTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function CrpBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<CrpTemplateSlug | null>(null);
  const toolConfig = AI_TOOL_CONFIGS['carbon-reduction-plan'];

  const handleTemplateSelect = (slug: CrpTemplateSlug) => {
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <CrpTemplatePicker onSelect={handleTemplateSelect} />
      </div>
    );
  }

  // Step 2+: Template selected → show the AI builder with back button
  const templateConfig = CRP_TEMPLATE_CONFIGS[selectedTemplate];
  ...
  return (
    <div>
      ...
      <AiToolBuilderClient
        ...
        crpTemplateSlug={selectedTemplate}
      />
    </div>
  );
}
