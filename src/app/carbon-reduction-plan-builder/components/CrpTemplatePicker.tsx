...
import { CRP_TEMPLATE_CONFIGS } from '@/lib/carbon-reduction/template-config';
import { CRP_TEMPLATE_ORDER } from '@/lib/carbon-reduction/template-config';
import type { CrpTemplateSlug } from '@/lib/carbon-reduction/types';
import CrpTemplatePreviewModal from './CrpTemplatePreviewModal';

interface CrpTemplatePickerProps {
  onSelect: (slug: CrpTemplateSlug) => void;
}

export default function CrpTemplatePicker({ onSelect }: CrpTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<CrpTemplateSlug | null>(null);
  ...
  const handleTemplateClick = (slug: CrpTemplateSlug) => {
    const templateStatus = getTemplateStatus();
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/carbon-reduction-plan-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: CrpTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a Carbon Reduction Plan Template</h2>
        {!isAuthenticated && (
          <p>Sign in to access templates and start generating carbon reduction plans</p>
        )}
        {isAuthenticated && !isPaid && (
          <p>Carbon Reduction Plan Builder is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>
        )}
        {isAuthenticated && isPaid && (
      ...
      <div className="template-grid-4">
        {CRP_TEMPLATE_ORDER.map((slug) => {
          const template = CRP_TEMPLATE_CONFIGS[slug];
      ...
      {/* Preview modal */}
      {previewSlug && (
        <CrpTemplatePreviewModal
          template={CRP_TEMPLATE_CONFIGS[previewSlug]}
