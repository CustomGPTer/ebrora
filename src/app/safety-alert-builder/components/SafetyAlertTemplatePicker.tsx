...
import { SAFETY_ALERT_TEMPLATE_ORDER, SAFETY_ALERT_FREE_TEMPLATES, SafetyAlertTemplateSlug } from '@/lib/safety-alert/types';
import SafetyAlertTemplatePreviewModal from './SafetyAlertTemplatePreviewModal';

interface SafetyAlertTemplatePickerProps {
  onSelect: (templateSlug: SafetyAlertTemplateSlug) => void;
}

export default function SafetyAlertTemplatePicker({ onSelect }: SafetyAlertTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<SafetyAlertTemplateSlug | null>(null);
  ...
  const canAccessTemplate = (slug: SafetyAlertTemplateSlug): boolean => {
    ...
    return SAFETY_ALERT_FREE_TEMPLATES.includes(slug);
  };

  const getTemplateStatus = (slug: SafetyAlertTemplateSlug): 'available' | 'locked-auth' | 'locked-upgrade' => {
    if (!isAuthenticated) return 'locked-auth';
    if (canAccessTemplate(slug)) return 'available';
    return 'locked-upgrade';
  };

  const handleTemplateClick = (slug: SafetyAlertTemplateSlug) => {
    const templateStatus = getTemplateStatus(slug);
    if (templateStatus === 'locked-auth') { router.push('/auth/login?callbackUrl=/safety-alert-builder'); return; }
    if (templateStatus === 'locked-upgrade') { router.push('/pricing'); return; }
    onSelect(slug);
  };

  const handlePreviewClick = (e: React.MouseEvent, slug: SafetyAlertTemplateSlug) => { e.stopPropagation(); setPreviewSlug(slug); };

  return (
    <div className="template-picker-section">
      <div className="template-picker-header">
        <h2>Choose a Safety Alert Template</h2>
        {!isAuthenticated && (<p>Sign in to access templates and start generating safety alert bulletins</p>)}
      ...
      <div className="template-grid-4">
        {SAFETY_ALERT_TEMPLATE_ORDER.map((slug) => {
          const template = SAFETY_ALERT_TEMPLATE_CONFIGS[slug];
          const templateStatus = getTemplateStatus(slug);
          const isLocked = templateStatus !== 'available';
          const isFreeTemplate = SAFETY_ALERT_FREE_TEMPLATES.includes(slug);
      ...
      {previewSlug && (<SafetyAlertTemplatePreviewModal template={SAFETY_ALERT_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />)}
    </div>
  );
}
