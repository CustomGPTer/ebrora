...
import { DELAY_TEMPLATE_ORDER } from '@/lib/delay/template-config';
import type { DelayTemplateSlug } from '@/lib/delay/types';
import DelayTemplatePreviewModal from './DelayTemplatePreviewModal';

interface DelayTemplatePickerProps {
  onSelect: (slug: DelayTemplateSlug) => void;
}

export default function DelayTemplatePicker({ onSelect }: DelayTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<DelayTemplateSlug | null>(null);
  ...
  const handleTemplateClick = (slug: DelayTemplateSlug) => {
    const templateStatus = getTemplateStatus();
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/delay-notification-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: DelayTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a Delay Notification Template</h2>
        {!isAuthenticated && (
          <p>Sign in to access templates and start generating scope documents</p>
        )}
        {isAuthenticated && !isPaid && (
          <p>Delay Notification Builder is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>
        )}
        {isAuthenticated && isPaid && (
      ...
      <div className="template-grid-3">
        {DELAY_TEMPLATE_ORDER.map((slug) => {
          const template = DELAY_TEMPLATE_CONFIGS[slug];
      ...
      {/* Preview modal */}
      {previewSlug && (
        <DelayTemplatePreviewModal
          template={DELAY_TEMPLATE_CONFIGS[previewSlug]}
