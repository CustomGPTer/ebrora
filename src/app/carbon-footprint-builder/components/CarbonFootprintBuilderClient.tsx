...
import { CarbonFootprintTemplateSlug } from '@/lib/carbon-footprint/types';
import { CARBON_FOOTPRINT_TEMPLATE_CONFIGS } from '@/lib/carbon-footprint/template-config';
import CarbonFootprintTemplatePicker from './CarbonFootprintTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function CarbonFootprintBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<CarbonFootprintTemplateSlug | null>(null);
  const toolConfig = AI_TOOL_CONFIGS['carbon-footprint'];

  const handleTemplateSelect = (slug: CarbonFootprintTemplateSlug) => {
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <CarbonFootprintTemplatePicker onSelect={handleTemplateSelect} />
      </div>
    );
  }

  // Step 2+: Template selected → show the AI builder with back button
  const templateConfig = CARBON_FOOTPRINT_TEMPLATE_CONFIGS[selectedTemplate];
  ...
  return (
    <div>
      ...
      <AiToolBuilderClient
        ...
        carbonFootprintTemplateSlug={selectedTemplate}
      />
    </div>
  );
}
