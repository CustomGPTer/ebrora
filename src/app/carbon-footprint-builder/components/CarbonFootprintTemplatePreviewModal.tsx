...
import { CarbonFootprintTemplateConfig } from '@/lib/carbon-footprint/types';

interface CarbonFootprintTemplatePreviewModalProps {
  template: CarbonFootprintTemplateConfig;
  onClose: () => void;
}

const LAYOUT_LABEL: Record<string, string> = {
  standard: 'Standard Layout',
  technical: 'PAS 2080 Technical Layout',
  compact: 'Compact Layout',
  audit: 'Audit-Ready Layout',
};

export default function CarbonFootprintTemplatePreviewModal({ template, onClose }: CarbonFootprintTemplatePreviewModalProps) {
  ...
  for (let i = 0; i < totalPages; i++) {
    if (i < realPreviews.length) {
      pages.push({ src: realPreviews[i], label: `Page ${i + 1}`, isPlaceholder: false });
    } else {
      pages.push({ src: null, label: `Page ${i + 1}`, isPlaceholder: true });
    }
  }
        ...
        <div className="tpm-layout">
          <div className="tpm-viewer">
            <div className="tpm-main-image">
              {activeIndex > 0 && (
                <button className="tpm-nav tpm-nav--left" onClick={() => setActiveIndex(prev => prev - 1)} aria-label="Previous page">
              ...
              {pages[activeIndex].isPlaceholder ? (
                <div className="tpm-placeholder">
                  <div className="tpm-placeholder-icon">
                    <svg viewBox="0 0 24 24" width="40" height="40" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="7" y1="8" x2="17" y2="8" /><line x1="7" y1="12" x2="17" y2="12" /><line x1="7" y1="16" x2="13" y2="16" /></svg>
                  </div>
                  <span className="tpm-placeholder-label">{pages[activeIndex].label}</span>
                  <span className="tpm-placeholder-sub">Preview not yet available</span>
                </div>
              ) : (
              ...
              {activeIndex < pages.length - 1 && (
                <button className="tpm-nav tpm-nav--right" onClick={() => setActiveIndex(prev => prev + 1)} aria-label="Next page">
              ...
              <div className="tpm-thumbstrip">
                {pages.map((page, i) => (
                  <button key={i} className={`tpm-thumb ${i === activeIndex ? 'tpm-thumb--active' : ''}`} onClick={() => setActiveIndex(i)} aria-label={page.label}>
                    {page.isPlaceholder ? (
                      <div className="tpm-thumb-placeholder"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="7" y1="9" x2="17" y2="9" /><line x1="7" y1="13" x2="17" y2="13" /></svg></div>
                    ) : (
                      <Image src={page.src!} alt={page.label} fill sizes="80px" style={{ objectFit: 'cover', objectPosition: 'top' }} />
                    )}
          ...
          <div className="tpm-details">
            ...
            <div className="tpm-meta">
              <span className="tpm-meta-pill">
                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="7" y1="8" x2="17" y2="8" /><line x1="7" y1="12" x2="17" y2="12" /><line x1="7" y1="16" x2="13" y2="16" /></svg>
                {template.pageCount} {template.pageCount === 1 ? 'page' : 'pages'}
              </span>
              <span className="tpm-meta-pill">
                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M3 9h18" /><path d="M9 3v18" /></svg>
                {LAYOUT_LABEL[template.layout] || template.layout}
              </span>
            </div>
