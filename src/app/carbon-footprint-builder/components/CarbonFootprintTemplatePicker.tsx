...
import { CARBON_FOOTPRINT_TEMPLATE_CONFIGS } from '@/lib/carbon-footprint/template-config';
import { CARBON_FOOTPRINT_TEMPLATE_ORDER, CARBON_FOOTPRINT_FREE_TEMPLATES, CarbonFootprintTemplateSlug } from '@/lib/carbon-footprint/types';
import CarbonFootprintTemplatePreviewModal from './CarbonFootprintTemplatePreviewModal';

interface CarbonFootprintTemplatePickerProps {
  onSelect: (templateSlug: CarbonFootprintTemplateSlug) => void;
}

export default function CarbonFootprintTemplatePicker({ onSelect }: CarbonFootprintTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<CarbonFootprintTemplateSlug | null>(null);
  ...
  const canAccessTemplate = (slug: CarbonFootprintTemplateSlug): boolean => {
    ...
    return CARBON_FOOTPRINT_FREE_TEMPLATES.includes(slug);
  };

  const getTemplateStatus = (slug: CarbonFootprintTemplateSlug): 'available' | 'locked-auth' | 'locked-upgrade' => {
    if (!isAuthenticated) return 'locked-auth';
    if (canAccessTemplate(slug)) return 'available';
    return 'locked-upgrade';
  };

  const handleTemplateClick = (slug: CarbonFootprintTemplateSlug) => {
    const templateStatus = getTemplateStatus(slug);
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/carbon-footprint-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: CarbonFootprintTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a Carbon Footprint Template</h2>
        {!isAuthenticated && (
          <p>Sign in to access templates and start generating carbon footprint assessments</p>
        )}
        {isAuthenticated && !isPaid && (
          <p>Free plan: 2 templates available &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade for all 4</a></p>
        )}
        {isAuthenticated && isPaid && (
          <p>All 4 templates available on your plan</p>
        )}
      </div>

      {/* Grid */}
      <div className="template-grid-4">
        {CARBON_FOOTPRINT_TEMPLATE_ORDER.map((slug) => {
          const template = CARBON_FOOTPRINT_TEMPLATE_CONFIGS[slug];
          const templateStatus = getTemplateStatus(slug);
          const isLocked = templateStatus !== 'available';
          const isFreeTemplate = CARBON_FOOTPRINT_FREE_TEMPLATES.includes(slug);
      ...
      {/* Preview modal */}
      {previewSlug && (
        <CarbonFootprintTemplatePreviewModal
          template={CARBON_FOOTPRINT_TEMPLATE_CONFIGS[previewSlug]}
