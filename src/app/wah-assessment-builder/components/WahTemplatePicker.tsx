...
import { WAH_TEMPLATE_CONFIGS, WAH_TEMPLATE_ORDER } from '@/lib/wah/template-config';
import type { WahTemplateSlug } from '@/lib/wah/types';
import WahTemplatePreviewModal from './WahTemplatePreviewModal';

interface Props { onSelect: (slug: WahTemplateSlug) => void; }

export default function WahTemplatePicker({ onSelect }: Props) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<WahTemplateSlug | null>(null);
  ...
  const handleClick = (slug: WahTemplateSlug) => {
    const s = getStatus();
    if (s === 'locked-auth') { router.push('/auth/login?callbackUrl=/wah-assessment-builder'); return; }
  ...
  return (
    <div className="template-picker-section">
      <div className="template-picker-header">
        <h2>Choose a Working at Height Assessment Template</h2>
        {!isAuthenticated && <p>Sign in to access templates and start generating documents</p>}
        {isAuthenticated && !isPaid && <p>WAH Assessment is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>}
        {isAuthenticated && isPaid && <p>All templates available on your plan</p>}
      </div>
      <div className="template-grid-4">
        {WAH_TEMPLATE_ORDER.map((slug) => {
          const template = WAH_TEMPLATE_CONFIGS[slug];
          return (
      ...
      {previewSlug && <WahTemplatePreviewModal template={WAH_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />}
    </div>
  );
}
