...
import { NCR_TEMPLATE_ORDER, NCR_FREE_TEMPLATES, NcrTemplateSlug } from '@/lib/ncr/types';
import NcrTemplatePreviewModal from './NcrTemplatePreviewModal';

interface NcrTemplatePickerProps {
  onSelect: (templateSlug: NcrTemplateSlug) => void;
}

export default function NcrTemplatePicker({ onSelect }: NcrTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<NcrTemplateSlug | null>(null);
  ...
  const canAccessTemplate = (slug: NcrTemplateSlug): boolean => {
    ...
    return NCR_FREE_TEMPLATES.includes(slug);
  };

  const getTemplateStatus = (slug: NcrTemplateSlug): 'available' | 'locked-auth' | 'locked-upgrade' => {
    if (!isAuthenticated) return 'locked-auth';
    if (canAccessTemplate(slug)) return 'available';
    return 'locked-upgrade';
  };

  const handleTemplateClick = (slug: NcrTemplateSlug) => {
    const templateStatus = getTemplateStatus(slug);
    if (templateStatus === 'locked-auth') { router.push('/auth/login?callbackUrl=/ncr-builder'); return; }
    if (templateStatus === 'locked-upgrade') { router.push('/pricing'); return; }
    onSelect(slug);
  };

  const handlePreviewClick = (e: React.MouseEvent, slug: NcrTemplateSlug) => { e.stopPropagation(); setPreviewSlug(slug); };

  return (
    <div className="template-picker-section">
      <div className="template-picker-header">
        <h2>Choose an NCR Template</h2>
        {!isAuthenticated && (<p>Sign in to access templates and start generating non-conformance reports</p>)}
        {isAuthenticated && !isPaid && (<p>Free plan: 2 templates available &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade for all 6</a></p>)}
        {isAuthenticated && isPaid && (<p>All 6 templates available on your plan</p>)}
      </div>

      <div className="template-grid-3">
        {NCR_TEMPLATE_ORDER.map((slug) => {
          const template = NCR_TEMPLATE_CONFIGS[slug];
          const templateStatus = getTemplateStatus(slug);
          const isLocked = templateStatus !== 'available';
          const isFreeTemplate = NCR_FREE_TEMPLATES.includes(slug);
      ...
      {previewSlug && (<NcrTemplatePreviewModal template={NCR_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />)}
    </div>
  );
}
