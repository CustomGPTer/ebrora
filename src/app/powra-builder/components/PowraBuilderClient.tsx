...
import { PowraTemplateSlug } from '@/lib/powra/types';
import { POWRA_TEMPLATE_CONFIGS } from '@/lib/powra/template-config';
import PowraTemplatePicker from './PowraTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function PowraBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<PowraTemplateSlug | null>(null);
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <PowraTemplatePicker onSelect={setSelectedTemplate} />
      </div>
    );
  }

  const templateConfig = POWRA_TEMPLATE_CONFIGS[selectedTemplate];
  return (
    <div>
      ...
      <AiToolBuilderClient toolConfig={toolConfig} powraTemplateSlug={selectedTemplate} />
    </div>
  );
}
