...
import { CdmCheckerTemplateSlug } from '@/lib/cdm-checker/types';
import { CDM_CHECKER_TEMPLATE_CONFIGS } from '@/lib/cdm-checker/template-config';
import CdmCheckerTemplatePicker from './CdmCheckerTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function CdmCheckerBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<CdmCheckerTemplateSlug | null>(null);
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
            ...
            {[1, 2, 3, 4].map((n) => (
              <div key={n} className={`rams-progress-step ${n <= 1 ? 'active' : ''}`}>
                ...
                <span className="rams-progress-label">
                  {n === 1 ? 'Choose Template' : n === 2 ? 'Describe Project' : n === 3 ? 'Interview' : 'Download'}
                </span>
              </div>
            ))}
        ...
        <CdmCheckerTemplatePicker onSelect={setSelectedTemplate} />
      </div>
    );
  }

  const templateConfig = CDM_CHECKER_TEMPLATE_CONFIGS[selectedTemplate];
  return (
    <div>
      ...
      <AiToolBuilderClient toolConfig={toolConfig} cdmCheckerTemplateSlug={selectedTemplate} />
    </div>
  );
}
