...
import { TRAFFIC_TEMPLATE_ORDER } from '@/lib/traffic/template-config';
import type { TrafficTemplateSlug } from '@/lib/traffic/types';
import TrafficTemplatePreviewModal from './TrafficTemplatePreviewModal';

interface TrafficTemplatePickerProps {
  onSelect: (slug: TrafficTemplateSlug) => void;
}

export default function TrafficTemplatePicker({ onSelect }: TrafficTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<TrafficTemplateSlug | null>(null);
  ...
  const handleTemplateClick = (slug: TrafficTemplateSlug) => {
    const templateStatus = getTemplateStatus();
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/traffic-management-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: TrafficTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a Traffic Management Plan Template</h2>
        {!isAuthenticated && (
          <p>Sign in to access templates and start generating scope documents</p>
        )}
        {isAuthenticated && !isPaid && (
          <p>Traffic Management Plan Builder is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>
        )}
        {isAuthenticated && isPaid && (
      ...
      <div className="template-grid-4">
        {TRAFFIC_TEMPLATE_ORDER.map((slug) => {
          const template = TRAFFIC_TEMPLATE_CONFIGS[slug];
      ...
      {/* Preview modal */}
      {previewSlug && (
        <TrafficTemplatePreviewModal
          template={TRAFFIC_TEMPLATE_CONFIGS[previewSlug]}
