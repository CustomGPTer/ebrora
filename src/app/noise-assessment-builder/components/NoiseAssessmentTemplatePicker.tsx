...
import { NOISE_ASSESSMENT_TEMPLATE_CONFIGS } from '@/lib/noise-assessment/template-config';
import { NOISE_ASSESSMENT_TEMPLATE_ORDER, NOISE_ASSESSMENT_FREE_TEMPLATES, NoiseAssessmentTemplateSlug } from '@/lib/noise-assessment/types';
import NoiseAssessmentTemplatePreviewModal from './NoiseAssessmentTemplatePreviewModal';

interface Props { onSelect: (slug: NoiseAssessmentTemplateSlug) => void; }

export default function NoiseAssessmentTemplatePicker({ onSelect }: Props) {
  ...
  const handleClick = (slug: NoiseAssessmentTemplateSlug) => { const s = getStatus(slug); if (s === 'locked-auth') { router.push('/auth/login?callbackUrl=/noise-assessment-builder'); return; } if (s === 'locked-upgrade') { router.push('/rams-builder/pricing'); return; } onSelect(slug); };
  return (
    <div className="template-picker-section">
      <div className="template-picker-header">
        <h2>Choose a Noise Assessment Template</h2>
        {!isAuthenticated && <p>Sign in to access templates and generate noise assessments</p>}
        {isAuthenticated && !isPaid && <p>Noise Assessment requires a paid plan &middot; <a href="/rams-builder/pricing" className="template-picker-upgrade-link">View plans</a></p>}
        {isAuthenticated && isPaid && <p>All 4 templates available on your plan</p>}
      </div>
      <div className="template-grid-5">
        {NOISE_ASSESSMENT_TEMPLATE_ORDER.map((slug) => {
          const tpl = NOISE_ASSESSMENT_TEMPLATE_CONFIGS[slug]; const st = getStatus(slug); const locked = st !== 'available'; const isFree = NOISE_ASSESSMENT_FREE_TEMPLATES.includes(slug);
          return (
      ...
      {previewSlug && <NoiseAssessmentTemplatePreviewModal template={NOISE_ASSESSMENT_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />}
    </div>
  );
}
