...
import { WASTE_TEMPLATE_ORDER } from '@/lib/waste/template-config';
import type { WasteTemplateSlug } from '@/lib/waste/types';
import WasteTemplatePreviewModal from './WasteTemplatePreviewModal';

interface WasteTemplatePickerProps {
  onSelect: (slug: WasteTemplateSlug) => void;
}

export default function WasteTemplatePicker({ onSelect }: WasteTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<WasteTemplateSlug | null>(null);
  ...
  const handleTemplateClick = (slug: WasteTemplateSlug) => {
    const templateStatus = getTemplateStatus();
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/waste-management-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: WasteTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a Waste Management Plan Template</h2>
        {!isAuthenticated && (
          <p>Sign in to access templates and start generating scope documents</p>
        )}
        {isAuthenticated && !isPaid && (
          <p>Waste Management Plan Builder is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>
        )}
        {isAuthenticated && isPaid && (
      ...
      <div className="template-grid-3">
        {WASTE_TEMPLATE_ORDER.map((slug) => {
          const template = WASTE_TEMPLATE_CONFIGS[slug];
      ...
      {/* Preview modal */}
      {previewSlug && (
        <WasteTemplatePreviewModal
          template={WASTE_TEMPLATE_CONFIGS[previewSlug]}
