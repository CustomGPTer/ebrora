...
import { RIDDOR_TEMPLATE_CONFIGS } from '@/lib/riddor/template-config';
import RiddorTemplatePicker from './RiddorTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function RiddorBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<RiddorTemplateSlug | null>(null);
  const toolConfig = AI_TOOL_CONFIGS['riddor-report'];
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <RiddorTemplatePicker onSelect={(slug) => setSelectedTemplate(slug)} />
      </div>
    );
  }

  const templateConfig = RIDDOR_TEMPLATE_CONFIGS[selectedTemplate];
  ...
  return (
    <div>
      ...
      <AiToolBuilderClient toolConfig={toolConfig} riddorTemplateSlug={selectedTemplate} />
    </div>
  );
}
