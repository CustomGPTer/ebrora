...
import { WBV_TEMPLATE_CONFIGS, WBV_TEMPLATE_ORDER } from '@/lib/wbv/template-config';
import type { WbvTemplateSlug } from '@/lib/wbv/types';
import WbvTemplatePreviewModal from './WbvTemplatePreviewModal';

interface Props { onSelect: (slug: WbvTemplateSlug) => void; }

export default function WbvTemplatePicker({ onSelect }: Props) {
  ...
  const handleClick = (slug: WbvTemplateSlug) => {
    const s = getStatus();
    if (s === 'locked-auth') { router.push('/auth/login?callbackUrl=/wbv-assessment-builder'); return; }
  ...
  return (
    <div className="template-picker-section">
      <div className="template-picker-header">
        <h2>Choose a Whole Body Vibration Assessment Template</h2>
        {!isAuthenticated && <p>Sign in to access templates and start generating documents</p>}
        {isAuthenticated && !isPaid && <p>WBV Assessment is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>}
        {isAuthenticated && isPaid && <p>All templates available on your plan</p>}
      </div>
      <div className="template-grid-3">
        {WBV_TEMPLATE_ORDER.map((slug) => {
          const template = WBV_TEMPLATE_CONFIGS[slug];
          return (
      ...
      {previewSlug && <WbvTemplatePreviewModal template={WBV_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />}
    </div>
  );
}
