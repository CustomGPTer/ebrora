...
import { WBV_TEMPLATE_CONFIGS } from '@/lib/wbv/template-config';
import WbvTemplatePicker from './WbvTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function WbvBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<WbvTemplateSlug | null>(null);
  const toolConfig = AI_TOOL_CONFIGS['wbv-assessment'];
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
            ...
            {[1, 2, 3, 4].map((n) => (
              <div key={n} className={`rams-progress-step ${n <= 1 ? 'active' : ''}`}>
                ...
                <span className="rams-progress-label">
                  {n === 1 ? 'Choose Template' : n === 2 ? 'Describe Vibration Exposure' : n === 3 ? 'Interview' : 'Download'}
                </span>
              </div>
            ))}
        ...
        <WbvTemplatePicker onSelect={(slug) => setSelectedTemplate(slug)} />
      </div>
    );
  }

  const templateConfig = WBV_TEMPLATE_CONFIGS[selectedTemplate];
  ...
  return (
    <div>
      ...
      <AiToolBuilderClient toolConfig={toolConfig} wbvTemplateSlug={selectedTemplate} />
    </div>
  );
}
