...
import { CoshhTemplateSlug } from '@/lib/coshh/types';
import { COSHH_TEMPLATE_CONFIGS } from '@/lib/coshh/template-config';
import CoshhTemplatePicker from './CoshhTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function CoshhBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<CoshhTemplateSlug | null>(null);
  const toolConfig = AI_TOOL_CONFIGS['coshh'];

  const handleTemplateSelect = (slug: CoshhTemplateSlug) => {
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <CoshhTemplatePicker onSelect={handleTemplateSelect} />
      </div>
    );
  }

  // Step 2+: Template selected → show the AI builder with back button
  const templateConfig = COSHH_TEMPLATE_CONFIGS[selectedTemplate];
  ...
  return (
    <div>
      ...
      <AiToolBuilderClient
        ...
        coshhTemplateSlug={selectedTemplate}
      />
    </div>
  );
}
