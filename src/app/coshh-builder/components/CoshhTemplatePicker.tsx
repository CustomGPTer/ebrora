...
import { COSHH_TEMPLATE_CONFIGS } from '@/lib/coshh/template-config';
import { COSHH_TEMPLATE_ORDER, COSHH_FREE_TEMPLATES, CoshhTemplateSlug } from '@/lib/coshh/types';
import CoshhTemplatePreviewModal from './CoshhTemplatePreviewModal';

interface CoshhTemplatePickerProps {
  onSelect: (templateSlug: CoshhTemplateSlug) => void;
}

export default function CoshhTemplatePicker({ onSelect }: CoshhTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<CoshhTemplateSlug | null>(null);
  ...
  const canAccessTemplate = (slug: CoshhTemplateSlug): boolean => {
    if (!isAuthenticated) return false;
    if (isPaid) return true;
    return COSHH_FREE_TEMPLATES.includes(slug);
  };

  const getTemplateStatus = (slug: CoshhTemplateSlug): 'available' | 'locked-auth' | 'locked-upgrade' => {
    if (!isAuthenticated) return 'locked-auth';
    if (canAccessTemplate(slug)) return 'available';
    return 'locked-upgrade';
  };

  const handleTemplateClick = (slug: CoshhTemplateSlug) => {
    const templateStatus = getTemplateStatus(slug);
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/coshh-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: CoshhTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a COSHH Assessment Template</h2>
        {!isAuthenticated && (
      ...
      <div className="template-grid-5">
        {COSHH_TEMPLATE_ORDER.map((slug) => {
          const template = COSHH_TEMPLATE_CONFIGS[slug];
          const templateStatus = getTemplateStatus(slug);
          const isLocked = templateStatus !== 'available';
          const isFreeTemplate = COSHH_FREE_TEMPLATES.includes(slug);
      ...
      {/* Preview modal */}
      {previewSlug && (
        <CoshhTemplatePreviewModal
          template={COSHH_TEMPLATE_CONFIGS[previewSlug]}
