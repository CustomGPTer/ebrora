...
import { PERMIT_TO_DIG_TEMPLATE_ORDER, PERMIT_TO_DIG_FREE_TEMPLATES, PermitToDigTemplateSlug } from '@/lib/permit-to-dig/types';
import PermitToDigTemplatePreviewModal from './PermitToDigTemplatePreviewModal';

interface Props { onSelect: (slug: PermitToDigTemplateSlug) => void; }

export default function PermitToDigTemplatePicker({ onSelect }: Props) {
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-grid-5">
        {PERMIT_TO_DIG_TEMPLATE_ORDER.map((slug) => {
          const tpl = PERMIT_TO_DIG_TEMPLATE_CONFIGS[slug]; const st = getStatus(slug); const locked = st !== 'available'; const isFree = PERMIT_TO_DIG_FREE_TEMPLATES.includes(slug);
          return (
      ...
      {previewSlug && <PermitToDigTemplatePreviewModal template={PERMIT_TO_DIG_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />}
    </div>
  );
}
