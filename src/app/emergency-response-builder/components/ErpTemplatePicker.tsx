...
import { ERP_TEMPLATE_CONFIGS } from '@/lib/erp/template-config';
import { ERP_TEMPLATE_ORDER, ERP_FREE_TEMPLATES, ErpTemplateSlug } from '@/lib/erp/types';
import ErpTemplatePreviewModal from './ErpTemplatePreviewModal';

interface Props { onSelect: (slug: ErpTemplateSlug) => void; }

export default function ErpTemplatePicker({ onSelect }: Props) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<ErpTemplateSlug | null>(null);
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-grid-5">
        {ERP_TEMPLATE_ORDER.map((slug) => {
          const tpl = ERP_TEMPLATE_CONFIGS[slug]; const st = getStatus(slug); const locked = st !== 'available'; const isFree = ERP_FREE_TEMPLATES.includes(slug);
          return (
      ...
      {previewSlug && <ErpTemplatePreviewModal template={ERP_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />}
    </div>
  );
}
