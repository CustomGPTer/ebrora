...
import { ErpTemplateSlug } from '@/lib/erp/types';
import { ERP_TEMPLATE_CONFIGS } from '@/lib/erp/template-config';
import ErpTemplatePicker from './ErpTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function ErpBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<ErpTemplateSlug | null>(null);
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <ErpTemplatePicker onSelect={setSelectedTemplate} />
      </div>
    );
  }

  const templateConfig = ERP_TEMPLATE_CONFIGS[selectedTemplate];
  return (
    <div>
      ...
      <AiToolBuilderClient toolConfig={toolConfig} erpTemplateSlug={selectedTemplate} />
    </div>
  );
}
