...
export default async function ToolboxCategoryPage({ params }: PageProps) {
  ...
  const totalTalks = cat.subfolders.reduce(
    (sum, s) => sum + s.talks.length,
    0
  );
  const totalExpected = cat.subfolders.reduce(
    (sum, s) => sum + s.expectedTalks.length,
    0
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="flex flex-wrap items-center gap-4 sm:gap-6 mb-8 pb-6 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">
              <strong className="text-gray-900">{cat.subfolders.length}</strong>{" "}
              sub-folder{cat.subfolders.length !== 1 ? "s" : ""}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-lg bg-[#1B5745]/8 flex items-center justify-center">
            ...
            <span className="text-sm text-gray-600">
              <strong className="text-gray-900">{totalTalks}</strong> available
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {cat.subfolders.map((sub) => {
            const available = sub.talks.length;
            const expected = sub.expectedTalks.length;
            ...
            return (
              ...
              >
                ...
                {/* Topic preview — first 3 expected talks */}
                <ul className="mb-3 space-y-0.5">
                  {sub.expectedTalks.slice(0, 3).map((t, i) => (
                    <li
                      key={i}
                  ...
                  {sub.expectedTalks.length > 3 && (
                    <li className="text-xs text-gray-300 italic pl-2.5">
                      + {sub.expectedTalks.length - 3} more
                    </li>
                  )}
                </ul>
