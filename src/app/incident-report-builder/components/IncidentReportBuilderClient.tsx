...
import { IncidentReportTemplateSlug } from '@/lib/incident-report/types';
import { INCIDENT_REPORT_TEMPLATE_CONFIGS } from '@/lib/incident-report/template-config';
import IncidentReportTemplatePicker from './IncidentReportTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function IncidentReportBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<IncidentReportTemplateSlug | null>(null);
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <IncidentReportTemplatePicker onSelect={setSelectedTemplate} />
      </div>
    );
  }

  const templateConfig = INCIDENT_REPORT_TEMPLATE_CONFIGS[selectedTemplate];
  return (
    <div>
      ...
      <AiToolBuilderClient toolConfig={toolConfig} incidentReportTemplateSlug={selectedTemplate} />
    </div>
  );
}
