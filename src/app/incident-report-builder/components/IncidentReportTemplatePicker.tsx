...
import { INCIDENT_REPORT_TEMPLATE_CONFIGS } from '@/lib/incident-report/template-config';
import { INCIDENT_REPORT_TEMPLATE_ORDER, INCIDENT_REPORT_FREE_TEMPLATES, IncidentReportTemplateSlug } from '@/lib/incident-report/types';
import IncidentReportTemplatePreviewModal from './IncidentReportTemplatePreviewModal';

interface Props { onSelect: (slug: IncidentReportTemplateSlug) => void; }

export default function IncidentReportTemplatePicker({ onSelect }: Props) {
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-grid-5">
        {INCIDENT_REPORT_TEMPLATE_ORDER.map((slug) => {
          const tpl = INCIDENT_REPORT_TEMPLATE_CONFIGS[slug]; const st = getStatus(slug); const locked = st !== 'available'; const isFree = INCIDENT_REPORT_FREE_TEMPLATES.includes(slug);
          return (
      ...
      {previewSlug && <IncidentReportTemplatePreviewModal template={INCIDENT_REPORT_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />}
    </div>
  );
}
