...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `Free ${category.name} ${FORMAT_LABEL[format]} Templates | Ebrora`,
    description: category.description || `Free ${FORMAT_LABEL[format]} templates for ${category.name}.`,
    openGraph: {
      title: `${category.name} ${FORMAT_LABEL[format]} Templates | Ebrora`,
            ...
            {category.templates.length === 0 ? (
              <div className="text-center py-16">
                ...
                <p className="text-sm text-gray-500 mt-1 max-w-md mx-auto">
                  We are preparing templates for this category. Check back soon or browse other categories.
                </p>
              </div>
            ) : (
