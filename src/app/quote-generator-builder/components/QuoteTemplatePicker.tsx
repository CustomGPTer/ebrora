...
import { QUOTE_TEMPLATE_CONFIGS, QUOTE_TEMPLATE_ORDER } from '@/lib/quote/template-config';
...
interface QuoteTemplatePickerProps {
  onSelect: (slug: QuoteTemplateSlug) => void;
}

export default function QuoteTemplatePicker({ onSelect }: QuoteTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<QuoteTemplateSlug | null>(null);
  ...
  const handleTemplateClick = (slug: QuoteTemplateSlug) => {
    const templateStatus = getTemplateStatus();
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/quote-generator-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: QuoteTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a Quotation Template</h2>
        {!isAuthenticated && (
          <p>Sign in to access templates and start generating quotation documents</p>
        )}
        {isAuthenticated && !isPaid && (
          <p>Quotation Builder is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>
        )}
        {isAuthenticated && isPaid && (
          <p>All 4 templates available on your plan &mdash; from full tender submissions to quick budget prices</p>
        )}
      </div>

      {/* Grid */}
      <div className="template-grid-4">
        {QUOTE_TEMPLATE_ORDER.map((slug) => {
          const template = QUOTE_TEMPLATE_CONFIGS[slug];
          ...
          return (
            ...
            >
              ...
              <div className="tpl-card-thumb">
                ...
                {/* Detail level badge */}
                <span className="tpl-card-detail-badge" style={{ backgroundColor: `#${template.accentColor}` }}>
      ...
      {previewSlug && (
        <QuoteTemplatePreviewModal
          template={QUOTE_TEMPLATE_CONFIGS[previewSlug]}
