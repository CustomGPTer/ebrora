...
import { QUOTE_TEMPLATE_CONFIGS } from '@/lib/quote/template-config';
...
export default function QuoteBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<QuoteTemplateSlug | null>(null);
  const toolConfig = AI_TOOL_CONFIGS['quote-generator'];

  const handleTemplateSelect = (slug: QuoteTemplateSlug) => {
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <QuoteTemplatePicker onSelect={handleTemplateSelect} />
      </div>
    );
  }

  // Step 2+: Template selected → show the AI builder with back button
  const templateConfig = QUOTE_TEMPLATE_CONFIGS[selectedTemplate];
  ...
  return (
    <div>
      ...
      <AiToolBuilderClient
        ...
        quoteTemplateSlug={selectedTemplate}
      />
    </div>
  );
}
