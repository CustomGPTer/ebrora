...
import { LiftPlanTemplateSlug } from '@/lib/lift-plan/types';
import { LIFT_PLAN_TEMPLATE_CONFIGS } from '@/lib/lift-plan/template-config';
import LiftPlanTemplatePicker from './LiftPlanTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function LiftPlanBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<LiftPlanTemplateSlug | null>(null);
          ...
          <div className="rams-progress-bar">
            {[1, 2, 3, 4].map((n) => (
              <div key={n} className={`rams-progress-step ${n <= 1 ? 'active' : ''}`}>
                ...
                <span className="rams-progress-label">{n === 1 ? 'Choose Template' : n === 2 ? 'Describe Lift' : n === 3 ? 'Interview' : 'Download'}</span>
              </div>
            ))}
            <div className="rams-progress-line"><div className="rams-progress-line-fill" style={{ width: '0%' }} /></div>
          </div>
        </div>
        <LiftPlanTemplatePicker onSelect={setSelectedTemplate} />
      </div>
    );
  }

  const templateConfig = LIFT_PLAN_TEMPLATE_CONFIGS[selectedTemplate];
  return (
    <div>
      ...
      <AiToolBuilderClient toolConfig={toolConfig} liftPlanTemplateSlug={selectedTemplate} />
    </div>
  );
}
