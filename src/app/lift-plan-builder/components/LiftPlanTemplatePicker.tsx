...
import { LIFT_PLAN_TEMPLATE_CONFIGS } from '@/lib/lift-plan/template-config';
import { LIFT_PLAN_TEMPLATE_ORDER, LIFT_PLAN_FREE_TEMPLATES, LiftPlanTemplateSlug } from '@/lib/lift-plan/types';
import LiftPlanTemplatePreviewModal from './LiftPlanTemplatePreviewModal';

interface Props { onSelect: (slug: LiftPlanTemplateSlug) => void; }

export default function LiftPlanTemplatePicker({ onSelect }: Props) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<LiftPlanTemplateSlug | null>(null);
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-grid-5">
        {LIFT_PLAN_TEMPLATE_ORDER.map((slug) => {
          const tpl = LIFT_PLAN_TEMPLATE_CONFIGS[slug]; const st = getStatus(slug); const locked = st !== 'available'; const isFree = LIFT_PLAN_FREE_TEMPLATES.includes(slug);
          return (
      ...
      {previewSlug && <LiftPlanTemplatePreviewModal template={LIFT_PLAN_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />}
    </div>
  );
}
