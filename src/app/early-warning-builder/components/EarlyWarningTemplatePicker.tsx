...
import { EARLY_WARNING_TEMPLATE_ORDER, EARLY_WARNING_FREE_TEMPLATES, EarlyWarningTemplateSlug } from '@/lib/early-warning/types';
import EarlyWarningTemplatePreviewModal from './EarlyWarningTemplatePreviewModal';

interface Props { onSelect: (slug: EarlyWarningTemplateSlug) => void; }

export default function EarlyWarningTemplatePicker({ onSelect }: Props) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<EarlyWarningTemplateSlug | null>(null);
  const isAuthenticated = status === 'authenticated';
  const userPlan = (session?.user as { subscriptionTier?: string })?.subscriptionTier || 'FREE';
  const isPaid = userPlan !== 'FREE';
  const canAccess = (slug: EarlyWarningTemplateSlug): boolean => { if (!isAuthenticated) return false; if (isPaid) return true; return EARLY_WARNING_FREE_TEMPLATES.includes(slug); };
  const getStatus = (slug: EarlyWarningTemplateSlug): 'available' | 'locked-auth' | 'locked-upgrade' => { if (!isAuthenticated) return 'locked-auth'; if (canAccess(slug)) return 'available'; return 'locked-upgrade'; };
  const handleClick = (slug: EarlyWarningTemplateSlug) => { const s = getStatus(slug); if (s === 'locked-auth') { router.push('/auth/login?callbackUrl=/early-warning-builder'); return; } if (s === 'locked-upgrade') { router.push('/pricing'); return; } onSelect(slug); };

  return (
    <div className="template-picker-section">
      <div className="template-picker-header">
        <h2>Choose an Early Warning Template</h2>
        {!isAuthenticated && <p>Sign in to access templates and generate NEC early warning notices</p>}
        {isAuthenticated && !isPaid && <p>2 free templates available &middot; Upgrade for all 8 &middot; <a href="/pricing" className="template-picker-upgrade-link">View plans</a></p>}
        {isAuthenticated && isPaid && <p>All 8 templates available on your plan</p>}
      </div>
      <div className="template-grid-5">
        {EARLY_WARNING_TEMPLATE_ORDER.map((slug) => {
          const tpl = EARLY_WARNING_TEMPLATE_CONFIGS[slug]; const st = getStatus(slug); const locked = st !== 'available'; const isFree = EARLY_WARNING_FREE_TEMPLATES.includes(slug);
          return (
      ...
      {previewSlug && <EarlyWarningTemplatePreviewModal template={EARLY_WARNING_TEMPLATE_CONFIGS[previewSlug]} onClose={() => setPreviewSlug(null)} />}
    </div>
  );
}
