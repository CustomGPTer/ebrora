...
import { INVASIVE_TEMPLATE_CONFIGS } from '@/lib/invasive/template-config';
import { INVASIVE_TEMPLATE_ORDER } from '@/lib/invasive/template-config';
import type { InvasiveTemplateSlug } from '@/lib/invasive/types';
import InvasiveTemplatePreviewModal from './InvasiveTemplatePreviewModal';

interface InvasiveTemplatePickerProps {
  onSelect: (slug: InvasiveTemplateSlug) => void;
}

export default function InvasiveTemplatePicker({ onSelect }: InvasiveTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<InvasiveTemplateSlug | null>(null);
  ...
  const handleTemplateClick = (slug: InvasiveTemplateSlug) => {
    const templateStatus = getTemplateStatus();
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/invasive-species-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: InvasiveTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a Invasive Species Management Plan Template</h2>
        {!isAuthenticated && (
          <p>Sign in to access templates and start generating scope documents</p>
        )}
        {isAuthenticated && !isPaid && (
          <p>Invasive Species Management Plan Builder is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>
        )}
        {isAuthenticated && isPaid && (
      ...
      <div className="template-grid-3">
        {INVASIVE_TEMPLATE_ORDER.map((slug) => {
          const template = INVASIVE_TEMPLATE_CONFIGS[slug];
      ...
      {/* Preview modal */}
      {previewSlug && (
        <InvasiveTemplatePreviewModal
          template={INVASIVE_TEMPLATE_CONFIGS[previewSlug]}
