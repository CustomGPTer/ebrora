...
import InvasiveTemplatePicker from './InvasiveTemplatePicker';
import AiToolBuilderClient from '@/components/ai-tools/AiToolBuilderClient';

export default function InvasiveBuilderClient() {
  const [selectedTemplate, setSelectedTemplate] = useState<InvasiveTemplateSlug | null>(null);
  const toolConfig = AI_TOOL_CONFIGS['invasive-species'];

  const handleTemplateSelect = (slug: InvasiveTemplateSlug) => {
  ...
  if (!selectedTemplate) {
    return (
      <div className="rams-builder">
        ...
        <InvasiveTemplatePicker onSelect={handleTemplateSelect} />
      </div>
    );
  }

  // Step 2+: Template selected → show the AI builder with back button
  const templateConfig = INVASIVE_TEMPLATE_CONFIGS[selectedTemplate];
  ...
  return (
    <div>
      ...
      <AiToolBuilderClient
        ...
        invasiveTemplateSlug={selectedTemplate}
      />
    </div>
  );
}
