...
import { VARIATION_TEMPLATE_ORDER } from '@/lib/variation/template-config';
import type { VariationTemplateSlug } from '@/lib/variation/types';
import VariationTemplatePreviewModal from './VariationTemplatePreviewModal';

interface VariationTemplatePickerProps {
  onSelect: (slug: VariationTemplateSlug) => void;
}

export default function VariationTemplatePicker({ onSelect }: VariationTemplatePickerProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [previewSlug, setPreviewSlug] = useState<VariationTemplateSlug | null>(null);
  ...
  const handleTemplateClick = (slug: VariationTemplateSlug) => {
    const templateStatus = getTemplateStatus();
    if (templateStatus === 'locked-auth') {
      router.push('/auth/login?callbackUrl=/variation-confirmation-builder');
      return;
    }
  ...
  const handlePreviewClick = (e: React.MouseEvent, slug: VariationTemplateSlug) => {
    e.stopPropagation();
    setPreviewSlug(slug);
  };
  ...
  return (
    <div className="template-picker-section">
      ...
      <div className="template-picker-header">
        <h2>Choose a Variation Confirmation Template</h2>
        {!isAuthenticated && (
          <p>Sign in to access templates and start generating scope documents</p>
        )}
        {isAuthenticated && !isPaid && (
          <p>Variation Confirmation Builder is available on paid plans &middot; <a href="/pricing" className="template-picker-upgrade-link">Upgrade to access</a></p>
        )}
        {isAuthenticated && isPaid && (
      ...
      <div className="template-grid-3">
        {VARIATION_TEMPLATE_ORDER.map((slug) => {
          const template = VARIATION_TEMPLATE_CONFIGS[slug];
      ...
      {/* Preview modal */}
      {previewSlug && (
        <VariationTemplatePreviewModal
          template={VARIATION_TEMPLATE_CONFIGS[previewSlug]}
